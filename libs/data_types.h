...
struct LogFileHeader {
    ...
    uint32_t    record_size_;     // Number of data bytes per record in this file or 0 for variable lengths.
...
inline size_t maxRecordSize(int size) {
    // In the worst case scenario, we will create two files (index, data) and add one record in each.
    return 2 * sizeof(LogFileHeader) + sizeof(VariableRecordIdx) + sizeof(VariableRecordDataHeader) + size;
}
