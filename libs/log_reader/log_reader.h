...
// This class spins a TCP server and listens for DataStreamsRequest or DataStream requests,
// handles them and responses with DataStreams or DataStreamPieces protos.
class LogReader {
    ...
    std::string handleReadingRequest(const std::string_view &data) {
        if (ready_) {
            ...
            if (request.ParseFromArray(data.data() ,data.size())) {
                ...
                if (request.has_file()) {
                    uint32_t start = request.has_start_idx() ? request.start_idx() : 0;
                    uint32_t max = request.has_max_count() ? request.max_count() : 0;
                    response = scanner_.getData(request.file(), request.start_ns(), request.end_ns(), start, max);
                } else {
                    response = scanner_.getStreams(request.start_ns(), request.end_ns());
                }
...
public:
    explicit LogReader(uint32_t port, const std::string &storage)
            : server_(port, std::bind(&LogReader::handleReadingRequest, this, std::placeholders::_1)), scanner_(storage), ready_(true) {}
