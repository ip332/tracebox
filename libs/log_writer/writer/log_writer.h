...
// This class stores incoming logging requests into a queue and implements a dedicated thread to flush the queue
// content into the file system using Storage class.
class LogWriter {
    // Thread controlling flag
    bool running_ = false;
    // All incoming requests will be added to the queue first.
    std::queue<LogRequest> queue_;
    // Push/pop operations will be invoked from different threads hence we need a mutex
    std::mutex mutex_;
    // A condition variable is required to wake up the writing thread when new item is added to the queue
    ...
    std::unique_ptr<LogRequest> wait() {
        ...
        condition_.wait(lock, [this] {
            return force_wakeup_ || !queue_.empty();
        });
